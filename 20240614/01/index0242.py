...
import data0242  # 修改為 data0242
from data0242 import FilterData, Info  # 從 data0242 匯入 FilterData 和 Info

class Window(ThemedTk):
    def __init__(self, theme: str = 'arc', **kwargs):  
        # -1-theme--#1-呼叫隱數(引數值呼叫)/建立參數 - default value
        super().__init__(theme=theme, **kwargs)  
        # -0-theme 傳回上面
        self.title('台北市YouBike2.0及時資料')
        try:
            # 呼叫DATA檔執行一次 #data 變數 #實體-attribute(可讀寫)/property（唯謮）/method
            self.__data = data0242.load_data()  
        except Exception as e:
            # 呼叫DATA檔-如果網址錯誤出現警告
            messagebox.showwarning(title='警告', message=str(e))
        
        # -1-呼叫  _display_interface 
    ...
    def item_selected(self, event):  
        ...
        for selected_item in tree.selection():  # elected_item所選項目
            ...
            # record記錄[1]欄位第二欄點資料會回傳
            print(site_data)
            # print(b)

def main():
    # 呼叫Window #breeze（ThemedTk視窗顏色)
