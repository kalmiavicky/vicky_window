...
# 設定字體
plt.title('SARIMA 模型時間序列分析與預測', fontproperties='SimHei')
...
def train_and_predict_sarima(df, output_dir):
    """
    使用SARIMA模型進行時間序列預測，並評估模型表現。

    參數:
    df (pd.DataFrame): 包含時間序列數據的DataFrame。
    ...
    # 預測訓練集和測試集
    y_pred_train = model_fit.predict(start=train.index[0], end=train.index[-1])
    y_pred_test = model_fit.predict(start=test.index[0], end=test.index[-1])
    ...
    plt.plot(train.index, train, label='訓練數據')
    plt.plot(test.index, test, label='測試數據')
    ...
    plt.xlabel('時間')
    plt.ylabel('平均價格 (元/公斤)')
    plt.title('SARIMA 模型時間序列分析與預測')
    plt.legend()

    # 儲存圖表
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    sarima_model_plot = os.path.join(output_dir, 'sarima_model_analysis.png')
    plt.savefig(sarima_model_plot)
    plt.close()

    return sarima_model_plot
...
@app.callback(
    [Output('台北二-normal-distribution', 'children'),
     ...
     Output('台北二-actual-predicted-plot', 'children')],
    Input('url', 'pathname')
)
def update_taipei_mk2_plots(pathname):
    ...
    if pathname == '/taipei_mk2_irwin':
        ...
        if df is not None:
            ...
            # 生成 SARIMA 模型的時間序列分析與預測數據
            SARIMA_model_path = '2024-M-2/analy_irwin_imgs/sarima_model_analysis_2.png'
            sarima_plot_path = train_and_predict_sarima(df, df, SARIMA_model_path)
            if os.path.exists(sarima_plot_path):
                sarima_plot_img = html.Img(src=f'/{sarima_plot_path}', style={'width': '100%'})
            else:
                sarima_plot_img = html.P("無法生成 SARIMA 模型圖", style={'textAlign': 'center', 'color': 'red'})

            return dist_img, box_plot_div, avg_price_table, sarima_plot_img
    
    return html.P("無法載入數據", style={'textAlign': 'center', 'color': 'gray'}), \
           html.P("無法載入數據", style={'textAlign': 'center', 'color': 'gray'}), \
           html.P("無法載入數據", style={'textAlign': 'center', 'color': 'gray'}), \
           html.P("無法載入數據", style={'textAlign': 'center', 'color': 'gray'})
