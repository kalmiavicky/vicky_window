...
class Window(ThemedTk):
    def __init__(self,theme:str|None,**kwargs): #父類別  自訂
        super().__init__(**kwargs)  #window裡面有個SELF
        self.title("BMI計算器")  
        ...
        self_height = ttk.Entry(input_frame,textvariable=self.hight_value)
        self_height.grid(row=1, column=1, padx=5, pady=5)
        ...
        self_weight = ttk.Entry(input_frame,textvariable=self.weight_value)
        self_weight.grid(row=2, column=1, padx=5, pady=5)    
    ...
    def show_result(self,name:str,height:int,weight:int):
            ...
            CustomMessagebox(self,title="BMI",name=name,bmi=bmi,status=status,advice=advice,status_color=status_color) #呼叫 所以用隱數名稱 呼叫  TOOS/.parent
