...
import taipei_mk1_chiinhwang  # 導入自定義模組 taipei_mk1_chiinhwang
import os
from jinja2 import Environment, FileSystemLoader
from img_i_2 import display_mango_images  # 導入圖片處理函數
...
def load_data(market):
    ...
    try:
        ...
        df = taipei_mk1_chiinhwang.taipei_mk1(file_path, market)  # 載入自定義模組的函數來處理市場數據
        all_descr, _, _, _, _, _ = taipei_mk1_chiinhwang.anal_mk1_data(df)  # 使用自定義模組分析數據
        return df, all_descr
...
def create_data_table(df, market):
    ...
    return html.Div([
        html.H2(f"金煌{market} 數據", style={'textAlign': 'center', 'marginBottom': '15px', 'fontSize': '20px'}),
...
def display_page(pathname):
    ...
    :return: 對應的 HTML 區段
    """
    if pathname == '/taipei_mk1_chiinhwang':
